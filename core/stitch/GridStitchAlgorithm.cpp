...
void GridStitchAlgorithm::setCropMargin(int pixels) {
    cropMargin_ = pixels;
}

void GridStitchAlgorithm::setCenterCropSize(int pixels) {
    centerCropSize_ = pixels;
}

cv::Mat GridStitchAlgorithm::stitch(const std::vector<cv::Mat>& images,
    ...
    try {
        ...
        // ── Canvas expansion: extract outer edge strips cropped by center crop ──
...
cv::Mat GridStitchAlgorithm::stitchWithPositions(const std::vector<cv::Mat>& images,
...
void GridStitchAlgorithm::computeTileSize(const cv::Size& imgSize, int& tileW, int& tileH) const {
    if (centerCropSize_ > 0) {
...
cv::Rect GridStitchAlgorithm::computeCropRoi(const cv::Size& imgSize, int tileW, int tileH,
                                              int row, int col,
...
cv::Mat GridStitchAlgorithm::cropTile(const cv::Mat& src, int tileW, int tileH,
                                       int row, int col,
                                       int gridRows, int gridCols,
                                       int ox, int oy) const {
